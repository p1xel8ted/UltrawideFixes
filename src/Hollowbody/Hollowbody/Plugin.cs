...
public class Plugin : BaseUnityPlugin
{
    ...
    internal static ConfigEntry<bool> Letterboxing { get; private set; }
    ...
    private void Awake()
    {
        ...
        UnityDebug = Config.Bind("00. General", "Unity Debug", false, new ConfigDescription("Enable Unity Debug logging.", null, new ConfigurationManagerAttributes { Order = 104 }));
        UnityDebug.SettingChanged += (_, _) => { UpdateDebug(); };

        UpdateDebug();

        SkipSplash = Config.Bind("00. General", "Skip Splash", true, new ConfigDescription("Skip the splash screen.", null, new ConfigurationManagerAttributes { Order = 105 }));
        ...
        AntiAliasing = Config.Bind("02. Quality", "Post-Process AA", PostProcessLayer.Antialiasing.TemporalAntialiasing, new ConfigDescription("Choose the anti-aliasing method applied during post-processing. Different methods offer various balances between performance and visual quality.", null, new ConfigurationManagerAttributes { Order = 97 }));
        AntiAliasing.SettingChanged += (_, _) => { UpdateGraphicsQualitySettings(); };

        CenterText = Config.Bind("04. UI", "Center Scene Title", false, new ConfigDescription("Center the scene title", null, new ConfigurationManagerAttributes { Order = 95 }));
        CenterText.SettingChanged += (_, _) => { Patches.UpdateTitleBars(); };

        CorrectUiElements = Config.Bind("04. UI", "Correct UI Elements", false, new ConfigDescription("Corrects certain UI elements to be constrained to 16:9 like the others.", null, new ConfigurationManagerAttributes { Order = 94 }));

        Letterboxing = Config.Bind("05. Camera", "Letterboxing", false, new ConfigDescription("Enable Letterboxing.", null, new ConfigurationManagerAttributes { Order = 96 }));
        Letterboxing.SettingChanged += (_, _) => { UpdateLetterbox(); };

        FollowCamera = Config.Bind("05. Camera", "Follow Camera", false, new ConfigDescription("Camera will follow the player instead of being room-based.", null, new ConfigurationManagerAttributes { Order = 93 }));
        ...
        AmbientOcclusion = Config.Bind("07. Post-Processing", "Ambient Occlusion", true, "Enable or disable Ambient Occlusion");
        ...
        // DepthOfField = Config.Bind("07. Post-Processing", "Depth of Field", false, "Enable or disable Depth of Field");
        // DepthOfField.SettingChanged += (_, _) => { UpdatePostProcessing(); };
        //
        ChromaticAberration = Config.Bind("07. Post-Processing", "Chromatic Aberration", false, "Enable or disable Chromatic Aberration");
        ...
        PictureCorrection = Config.Bind("07. Post-Processing", "Picture Correction", true, "Enable or disable Picture Correction");
        PictureCorrection.SettingChanged += (_, _) => { UpdatePostProcessing(); };
        
        RunInBackground = Config.Bind("08. Misc", "Run In Background", true, new ConfigDescription("Allows the game to run even when not in focus.", null, new ConfigurationManagerAttributes { Order = 90 }));
        RunInBackground.SettingChanged += (_, _) => { Application.runInBackground = RunInBackground.Value; };
        
        
        MuteInBackground = Config.Bind("08. Misc", "Mute In Background", false, new ConfigDescription("Mutes the game's audio when it is not in focus.", null, new ConfigurationManagerAttributes { Order = 89 }));
    ...
    private static void UpdateLetterbox()
    {
        Patches.UpdateLetterbox();
    }
    ...
    private static void UpdateFixedDeltaTime()
    {
        ...
        if (Mathf.Approximately(Screen.currentResolution.refreshRate, OriginalFixedDeltaTime.Value) || OriginalFixedDeltaTime.Value > Screen.currentResolution.refreshRate)
        {
        ...
        if (CorrectFixedUpdateRate.Value)
        {
            ...
            {
                Time.fixedDeltaTime = 1f / Utils.FindLowestFrameRateMultipleAboveFifty(Screen.currentResolution.refreshRate);
            }
        }
        else
        {
