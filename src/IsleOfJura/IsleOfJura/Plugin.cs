...
namespace IsleOfJura;

[BepInPlugin(PluginGuid, PluginName, PluginVersion)]
public class Plugin : BaseUnityPlugin
{
    // Plugin Metadata
    private const string PluginGuid = "p1xel8ted.isleofjura.uwfixes";
    private const string PluginName = "Isle of Jura Ultra-Wide";
    ...
    public static ConfigEntry<float> TerrainDetailObjectDensityMultiplier { get; private set; }
    public static ConfigEntry<float> TerrainDetailObjectDistanceMultiplier { get; private set; }
    ...
    {
        TerrainDetailObjectDensityMultiplier = Config.Bind("03. Graphics", "Terrain Detail Object Density Multiplier", 1f,
            new ConfigDescription(
                "Adjust the density of terrain detail objects. Higher values increase the number of objects, potentially improving visual quality but reducing performance.",
                new AcceptableValueRange<float>(0.1f, 10f),
                new ConfigurationManagerAttributes { Order = 87 }));
        TerrainDetailObjectDensityMultiplier.SettingChanged += (_, _) => Patches.Patches.UpdateGraphics();
        
        TerrainDetailObjectDistanceMultiplier = Config.Bind("03. Graphics", "Terrain Detail Object Distance Multiplier", 1f,
            new ConfigDescription(
                "Adjust the distance at which terrain detail objects are rendered. Higher values increase the rendering distance, potentially improving visual quality but reducing performance.",
                new AcceptableValueRange<float>(0.1f, 10f),
                new ConfigurationManagerAttributes { Order = 86 }));
        TerrainDetailObjectDistanceMultiplier.SettingChanged += (_, _) => Patches.Patches.UpdateGraphics();
        
        FieldOfViewMultiplier = Config.Bind("03. Graphics", "Field of View Multiplier", 1f,
            new ConfigDescription(
                "Adjust the field of view (FOV) for a wider or narrower perspective. Higher values provide a wider view, while lower values zoom in.",
                new AcceptableValueRange<float>(0.10f, 5f),
