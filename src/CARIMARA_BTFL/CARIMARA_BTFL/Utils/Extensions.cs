...
public static class Extensions
{
    ...
    internal static float FindLowestFrameRateMultipleAboveFifty(float originalRate)
    {
        ...
        // Validate input
        if (intRate <= Constants.MinimumPhysicsRate) return intRate;

        // Find the lowest divisor above minimum threshold
        for (var rate = intRate / 2; rate > Constants.MinimumPhysicsRate; rate--)
        {
            if (intRate % rate == 0)
            {
                return rate;
