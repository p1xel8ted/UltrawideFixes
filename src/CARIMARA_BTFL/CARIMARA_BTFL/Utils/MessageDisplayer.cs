...
public class MessageDisplayer : MonoBehaviour
{
    ...
    private void Initialize()
    {
        ...
        foreach (Corner corner in Enum.GetValues(typeof(Corner)))
        {
            ...
            var offset = Constants.MessageUI.PositionOffset;
            switch (corner)
            {
                case Corner.TopLeft:
                    ConfigureGroupLayout(groupRect, TextAnchor.UpperLeft, new Vector2(0, 1), new Vector2(offset, -offset));
                    break;
                case Corner.TopRight:
                    ConfigureGroupLayout(groupRect, TextAnchor.UpperRight, new Vector2(1, 1), new Vector2(-offset, -offset));
                    break;
                case Corner.BottomLeft:
                    ConfigureGroupLayout(groupRect, TextAnchor.LowerLeft, new Vector2(0, 0), new Vector2(offset, offset));
                    break;
                case Corner.BottomRight:
                    ConfigureGroupLayout(groupRect, TextAnchor.LowerRight, new Vector2(1, 0), new Vector2(-offset, offset));
                    break;
                default:
                    ConfigureGroupLayout(groupRect, TextAnchor.UpperLeft, new Vector2(0, 1), new Vector2(offset, -offset));
                    break;
    ...
    /// <summary>
    /// Configures the layout group for a specific corner.
    /// </summary>
    private static void ConfigureGroupLayout(RectTransform groupRect, TextAnchor alignment, Vector2 anchor, Vector2 position)
    {
        ...
        layoutGroup.spacing = Constants.MessageUI.LayoutSpacing;
    ...
    public void ShowMessage(string message, float duration = 2f, Corner corner = Corner.TopRight, int fontSize = 24, Color? color = null)
    {
        ...
        {
            Plugin.Log.LogError($"LayoutGroup for corner {corner} is not initialized.");
            return;
        }
        ...
        var maxWidth = Screen.width * Constants.MessageUI.MaxWidthPercentage;
