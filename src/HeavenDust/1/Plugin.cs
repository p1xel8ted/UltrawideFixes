...
public class Plugin : BaseUnityPlugin
{
    ...
    private const string PluginName = "Heaven Dust Ultra-Wide";
    ...
    private static float NativeWidth => Mathf.RoundToInt(MainHeight * BaseAspectRatio);
    ...
    private static ConfigEntry<bool> EnableCameraRotation { get; set; }
    ...
    private void Awake()
    {
        ...
        EnableCameraRotation = Config.Bind("02. Camera", "Enable Camera Rotation", true, new ConfigDescription("Enable camera rotation.", null, new ConfigurationManagerAttributes {Order = 106}));
        EnableCameraRotation.SettingChanged += (_, _) =>
        {
            UpdateCamera();
        };
        ...
        RunInBackground = Config.Bind("05. Misc", "Run In Background", true, new ConfigDescription("Allows the game to run even when not in focus.", null, new ConfigurationManagerAttributes {Order = 99}));
        RunInBackground.SettingChanged += (_, _) =>
        {
            Application.runInBackground = RunInBackground.Value;
        };

        MuteInBackground = Config.Bind("05. Misc", "Mute In Background", false, new ConfigDescription("Mutes the game's audio when it is not in focus.", null, new ConfigurationManagerAttributes {Order = 98}));
    ...
    private static void UpdateFixedDeltaTime()
    {
        ...
        if (CorrectFixedUpdateRate.Value)
        {
            ...
            {
                Time.fixedDeltaTime = 1f / HeavenDustOne.Misc.Utils.FindLowestFrameRateMultipleAboveFifty(MaxRefresh);
            }
        }
        else
        {
