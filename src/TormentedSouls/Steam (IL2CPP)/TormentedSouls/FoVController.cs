...
public class FoVController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        Plugin.Logger.LogInfo($"FoVController (Start) - { _virtualCamera.name}. Cams Initial FoV: { _fov.Value}");
    }
    private void OnEnable()
    {
        _virtualCamera = GetComponent<CinemachineVirtualCamera>();
        _fov.Value = _virtualCamera.m_Lens.FieldOfView;
        Plugin.Logger.LogInfo($"FoVController (OnEnable) - { _virtualCamera.name}. Cams Initial FoV: { _fov.Value}");
    }
