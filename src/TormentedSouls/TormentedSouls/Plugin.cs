...
public class Plugin : BasePlugin
{
    ...
    public override void Load()
    {
        ...
        CorrectFixedUpdateRate = Config.Bind("02. Performance", "Modify Physics Rate", false, new ConfigDescription("Adjust the fixed update rate to reduce camera judder based on your refresh rate. This may affect the game in unexpected ways.", null, new ConfigurationManagerAttributes {Order = 14}));
        CorrectFixedUpdateRate.SettingChanged += (_, _) => UpdateFixedDeltaTime();

        UseRefreshRateForFixedUpdateRate = Config.Bind("02. Performance", "Use Refresh Rate for Physics Rate", false, new ConfigDescription("Set the fixed update rate based on the monitor's refresh rate for smoother gameplay. May impact performance on lower-end systems.", null, new ConfigurationManagerAttributes {Order = 13}));
        UseRefreshRateForFixedUpdateRate.SettingChanged += (_, _) => UpdateFixedDeltaTime();

        AnisotropicFiltering = Config.Bind("02. Quality", "Anisotropic Filtering", true, new ConfigDescription("Enable Anisotropic Filtering.", null, new ConfigurationManagerAttributes {Order = 12}));
        ...
        PostProcessingEffects = Config.Bind("03. Post Processing", "Post Processing Effects", true, new ConfigDescription("Allow modification of Post Processing Effects. You can switch this off after making changes if you notice your FPS drop.", null, new ConfigurationManagerAttributes {Order = 11}));

        AmbientOcclusion = Config.Bind("03. Post Processing Effects", "Ambient Occlusion", true, new ConfigDescription("Enable Ambient Occlusion Post Processing Effect.", null, new ConfigurationManagerAttributes {Order = 10}));
        AmbientOcclusion.SettingChanged += (_, _) => EnablePostProcessEditing();

        Bloom = Config.Bind("03. Post Processing Effects", "Bloom", true, new ConfigDescription("Enable Bloom Post Processing Effect.", null, new ConfigurationManagerAttributes {Order = 9}));
        Bloom.SettingChanged += (_, _) => EnablePostProcessEditing();

        ColorAdjustments = Config.Bind("03. Post Processing Effects", "Color Adjustments", true, new ConfigDescription("Enable Color Adjustments Post Processing Effect.", null, new ConfigurationManagerAttributes {Order = 8}));
        ColorAdjustments.SettingChanged += (_, _) => EnablePostProcessEditing();

        RapidOldTVVignette = Config.Bind("03. Post Processing Effects", "Rapid Old TV Vignette", true, new ConfigDescription("Enable Rapid Old TV Vignette Post Processing Effect.", null, new ConfigurationManagerAttributes {Order = 7}));
        RapidOldTVVignette.SettingChanged += (_, _) => EnablePostProcessEditing();

        ScreenSpaceReflections = Config.Bind("03. Post Processing Effects", "Screen Space Reflections", true, new ConfigDescription("Enable Screen Space Reflections Post Processing Effect.", null, new ConfigurationManagerAttributes {Order = 6}));

        Sharpen = Config.Bind("03. Post Processing Effects", "Sharpen", true, new ConfigDescription("Enable Sharpen Post Processing Effect.", null, new ConfigurationManagerAttributes {Order = 5}));
        Sharpen.SettingChanged += (_, _) => EnablePostProcessEditing();

        Streak = Config.Bind("03. Post Processing Effects", "Streak", true, new ConfigDescription("Enable Streak Post Processing Effect.", null, new ConfigurationManagerAttributes {Order = 4}));
        Streak.SettingChanged += (_, _) => EnablePostProcessEditing();

        TiltShiftBlur = Config.Bind("03. Post Processing Effects", "Tilt Shift Blur", true, new ConfigDescription("Enable Tilt Shift Blur Post Processing Effect.", null, new ConfigurationManagerAttributes {Order = 3}));
