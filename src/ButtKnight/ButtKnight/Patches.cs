...
public static class Patches
{
    ...
    private static readonly Dictionary<int, Vector3> TransformScaleBackups = new();
    ...
    internal static void CorrectTransformScale(Transform tr, float buffer = 0f)
    {
        ...
        if (!TransformScaleBackups.TryGetValue(tr.GetInstanceID(), out var originalScale))
        {
            TransformScaleBackups.Add(tr.GetInstanceID(), tr.localScale);
        ...
        if (buffer != 0)
        {
            newX = originalScale.x * (Resolutions.PositiveScaleFactor + buffer);
        }
        else
        {
            if (originalScale.x != 0)
            {
                newX = originalScale.x * Resolutions.PositiveScaleFactor;
            }
            else
            {
                newX = Resolutions.PositiveScaleFactor;
            }
        }

        tr.localScale = new Vector3(newX, originalScale.y, originalScale.z);
    }
    ...
    public static float GetAdjustedXOffset(float originalValue)
    {
        var value = originalValue * Resolutions.PositiveScaleFactor;
        Plugin.Log.LogWarning($"Adjusting X offset from {originalValue} to {value} for aspect ratio {Resolutions.CurrentAspect}");
        return value;
    }
    ...
    public static void BossCutins_Start(BossCutins __instance)
    {
    ...
    public static void WorldStageController_Start(WorldStageController __instance)
    {
        ...
        if (buffer >= 0)
        {
            foreach (var name in new[] { "GradiantL", "GradiantR" })
            {
                var gradient = __instance.transform.Find(name);
                if (gradient)
                {
                    CorrectTransformScale(gradient, buffer);
                }
            }
        }

        var colorBg = __instance.transform.Find("ColorBg");
        if (colorBg)
        {
    ...
    public static void RevealableTextPrinterPanel_OnEnable(RevealableTextPrinterPanel __instance)
    {
        ...
        if (gradientOverlay)
        {
            CorrectTransformScale(gradientOverlay);
        }
    }
    ...
    public static void IntroScene_Start(IntroScene __instance)
    {
