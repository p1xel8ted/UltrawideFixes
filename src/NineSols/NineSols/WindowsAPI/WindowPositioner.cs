...
public class WindowPositioner : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        {
            denominator = 1,
            numerator = (uint)(Plugin.UseCustomRefreshRate.Value ? Plugin.CustomRefreshRate.Value : Plugin.MaxRefresh)
        };
        Display.displays[Plugin.DisplayToUse.Value].Activate(Plugin.MainWidth, Plugin.MainHeight, rr);
    ...
    private static void MoveWindowToDisplay(int displayIndex)
    {
        ...
        var window = FindWindow(null, "NineSols");
