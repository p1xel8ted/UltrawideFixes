...
public class Plugin : BasePlugin
{
    ...
    private const float NativeAspect = 16.0f / 9.0f;
    ...
    //
    //
    private static readonly string[] MeshRendersToProcess = ["Skies", "Seas"];
    //
    //
    private static int SimplifiedWidth => Util.GetGcd(MainWidth, MainHeight).simplifiedWidth;
    private static int SimplifiedHeight => Util.GetGcd(MainWidth, MainHeight).simplifiedHeight;
    //
    internal static float MainAspectRatio => MainWidth / (float)MainHeight;
     // public static ManualLogSource Logger { get; set; }
     private static float PositiveScaleFactor => MainAspectRatio / NativeAspect;
    ...
    // internal static int MainHeight => SelectedResolution.height;
     ...
     internal static int MainWidth => Screen.currentResolution.width;
     internal static int MainHeight => Screen.currentResolution.height;
    ...
    public static Vector2 SimplifiedMainAspect => new(SimplifiedWidth, SimplifiedHeight);
   // private static bool RequiresUpdate { get; set; }
    ...
    private static float CurrentAspect => (float)Screen.currentResolution.width / Screen.currentResolution.height;
    ...
    private static readonly Dictionary<int, float> OriginalMeshRenderScales = new();

    private static void RunMeshFixes()
    {
        ...
        foreach (var mr in meshRenderers)
        {
            ...
            var id = mr.GetInstanceID();
            if (!OriginalMeshRenderScales.TryGetValue(id, out var originalScale))
            {
                OriginalMeshRenderScales.Add(id, mr.transform.localScale.x);
                originalScale = mr.transform.localScale.x;
            }

            var x = originalScale * PositiveScaleFactor;
    ...
    // private static readonly WriteOnceFloat OriginalBackdropEndingVerticalSlice = new();
    ...
    private static readonly Dictionary<int, float> OriginalMountainsScale = new();

    private static void FixMountains()
    {
        var mountains = GameObject.Find("Level/Statics/Vista/Backdrop ViewPoint/Backdrop Ending Vertical Slice");
        if (mountains)
        {
            foreach (var transform in mountains.GetComponentsInChildren<Transform>())
            {
                if (transform.name.Equals("End_Scene_01-foggy", StringComparison.OrdinalIgnoreCase) || transform.name.StartsWith("mountain", StringComparison.OrdinalIgnoreCase))
                {
                    if (transform.name.Equals("Mountain01 (3)"))
                    {
                    ...
                    if (!OriginalMountainsScale.TryGetValue(id, out var originalScale))
                    {
                        OriginalMountainsScale.Add(id, transform.localScale.x);
