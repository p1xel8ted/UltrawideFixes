...
namespace OnceUponKatamari.UI.Patches;
...
public static class CanvasScalerPatches
{
    ...
    public static void CanvasScaler_OnEnable(CanvasScaler __instance)
    {
        ...
        var letterBox = __instance.transform.FindChild("LetterBoxParent");
        if (letterBox)
        {
            var header = letterBox.FindChild("Header");
            if (header)
            {
                header.gameObject.TryAddComponent<ForceImageOff>();
            }

            var footer = letterBox.FindChild("Footer");
            if (footer)
            {
                footer.gameObject.TryAddComponent<ForceImageOff>();
            }
        }
    }

    internal class ForceImageOff : MonoBehaviour
    {
    ...
    internal static void UpdateScalers(float aspect)
    {
        ...
        var validInstanceIds = new HashSet<int>(CanvasScalers.Select(scaler => scaler.GetInstanceID()));
        var keysToRemove = OriginalScreenMatchModes.Keys.Where(id => !validInstanceIds.Contains(id)).ToList();
