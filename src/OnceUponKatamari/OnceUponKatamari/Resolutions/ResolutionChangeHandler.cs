...
public static class ResolutionChangeHandler
{
    ...
    {
        __state = new Vector2(ResolutionManager.NativeDisplayWidth, ResolutionManager.NativeDisplayHeight);
    }
    ...
    public static void SetResolution_Postfix(int width, int height, Vector2 __state)
    {
        ...
        {
            Plugin.Logger.LogDebug($"Ignoring duplicate resolution change to {newRes.x}x{newRes.y} in same frame");
            return;
        }
        ...
        Plugin.Logger.LogInfo($"Resolution changed from {__state.x}x{__state.y} to {newRes.x}x{newRes.y}");
