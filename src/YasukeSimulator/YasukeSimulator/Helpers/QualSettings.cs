...
public static class QualSettings
{
    internal static void UpdateSettings()
    {
        ...
        foreach (var cam in Resources.FindObjectsOfTypeAll<Camera>())
        {
            if (cam)
            {
                cam.allowMSAA = Plugin.AntialiasingModeConfig.Value != AntialiasingMode.TemporalAntiAliasing; // Disable MSAA for TAA
                var extraData = cam.GetUniversalAdditionalCameraData();
                if (extraData)
                {
                    extraData.antialiasing = Plugin.AntialiasingModeConfig.Value;
