...
{
    private const string PluginGuid = "p1xel8ted.blueprince.ultrawide";
    private const string PluginName = "Blue Prince Ultra-Wide";
    ...
    {
        //Logger.LogWarning($"AspectUtility: {AspectUtility.wantedAspectRatio}, {AspectUtility.screenWidth}, {AspectUtility.screenHeight}, {AspectUtility.screenRect}");
        UpdateAll();

        var cameras = Resources.FindObjectsOfTypeAll<Camera>();
        foreach (var cam in cameras.ToList())
        {
            Plugin.Logger.LogWarning($"Cam: {cam.name}, {cam.aspect}, {cam.pixelRect}, {cam.rect}");
        }
    }
}