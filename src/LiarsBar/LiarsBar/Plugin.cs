...
{
    private const string PluginGuid = "p1xel8ted.liarsbar.uwfixes";
    private const string PluginName = "Liar's Bar Ultra-Wide";
    ...
    private void Awake()
    {
        ...
        var resList = Screen.resolutions.Select(res => $"{res.width}x{res.height}").Distinct().ToArray();
        ...
        Resolution = Config.Bind("01. Display", "Resolution", $"{MainWidth}x{MainHeight}", new ConfigDescription("Set the resolution to use.", new AcceptableValueList<string>(resList), new ConfigurationManagerAttributes { Order = 97 }));
    ...
    private static void UpdateDisplay()
    {
        ...
        Screen.SetResolution(res.width, res.height, FullScreenModeConfig.Value, RefreshRateRatio);
