...
public static class Patches
{
    ...
    private static HeroController HeroControllerInstance { get; set; }
    ...
    internal static void UpdateHeroSettings()
    {
        if (!HeroControllerInstance)
        {
            HeroControllerInstance = Object.FindFirstObjectByType<HeroController>();
            return;
        }

        if (HeroControllerInstance.heroLight)
        {
            OriginalAlpha.Value = HeroControllerInstance.heroLight.Alpha;

            HeroControllerInstance.heroLight.Alpha = Plugin.HeroLight.Value ? OriginalAlpha.Value : 0f;
        }

        DonutLightTransform ??= HeroControllerInstance.transform.Find("HeroLight/white_light_donut")?.GetComponent<SpriteRenderer>();
        if (DonutLightTransform)
        {
            DonutLightTransform.enabled = Plugin.HeroLightDonut.Value;
        }

        if (HeroControllerInstance.vignette)
        {
            HeroControllerInstance.vignette.enabled = Plugin.Vignette.Value;
        }
    }
    ...
    public static void HeroController_Start(HeroController __instance)
    {
        HeroControllerInstance = __instance;
        UpdateHeroSettings();
    }
    ...
    {
        if (__instance._unityCamera && HeroController._instance)
        {
