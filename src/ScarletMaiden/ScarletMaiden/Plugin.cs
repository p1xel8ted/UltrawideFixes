...
{
    private const string PluginGuid = "p1xel8ted.scarletmaiden.ultrawide";
    ...
    private void Awake()
    {
        ...
        CameraConfig.Initialize(Config);
        ...
        CameraConfig.SubscribeToEvents((_, _) => CameraPatches.UpdateCameras());
    ...
    private static void UpdateDisplay(bool force = false)
    {
        ...
        if (force)
        {
            Screen.SetResolution(ResolutionManager.SelectedResolution.width, ResolutionManager.SelectedResolution.height, DisplayConfig.FullScreenMode.Value, ResolutionManager.MaxRefresh);
