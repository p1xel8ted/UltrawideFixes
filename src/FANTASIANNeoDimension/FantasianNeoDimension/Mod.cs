using FANTASIANNeoDimension;

[assembly: MelonInfo(typeof(Mod), "FANTASIAN Neo Dimension Ultra-Wide", "0.1.0", "p1xel8ted")]
...
public class Mod : MelonMod
{
    ...
    private static void UpdateCameras()
    {
        ...
        foreach (var camera in cameras)
        {
            ...
            camera.pixelRect = new Rect(0, 0, NativeDisplayWidth, NativeDisplayHeight);
            MelonLogger.Warning($"Updated Camera: {camera.name}, Aspect={camera.aspect}, PixelRect={camera.pixelRect}");
            if (camera.targetTexture)
            {
                camera.targetTexture.Release();
                camera.targetTexture.width = NativeDisplayWidth;
                camera.targetTexture.height = NativeDisplayHeight;
                camera.targetTexture.Create();
                MelonLogger.Warning($"-- Updated Camera Target Texture: {camera.targetTexture.name}, Size={camera.targetTexture.width}x{camera.targetTexture.height}");
            }
            var camClipper = camera.GetComponent<CameraClipper>();
            {
                camClipper.enabled = false;
                MelonLogger.Warning($"-- Disabled CameraClipper on Camera: {camera.name}");
            }
        }
    }
    //
