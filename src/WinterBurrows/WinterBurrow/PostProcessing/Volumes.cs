...
using WinterBurrow.Utils;

namespace WinterBurrow.PostProcessing;
    ...
    internal static void ProcessVolumeRegistration(Volume volume)
    {
        foreach (var vol in volume.profile.components)
        {
            ...
            if (ConfigEntriesGlobal.TryAdd(name, configEntryGlobal))
            {
                ...
                MessageDisplayer.Instance.ShowMessage($"Registered '{name}' configuration entry...", 5, MessageDisplayer.Corner.TopRight, 15, Color.white);
            }
        }
    }
